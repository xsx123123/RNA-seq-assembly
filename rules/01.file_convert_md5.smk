...
rule seq_preprocessor_short_read:
    ...
    benchmark:
        "../benchmarks/seq_preprocessor_short_read.txt",
    params:
        raw_data_path = config['raw_data_path'],
        md5 = config['short_read_md5'],
    log:
        "../logs/01.qc/seq_preprocessor_short_read.txt",
    threads: 1
    shell:
        """
...
# ------------- long read check ------------ #
if config['long_read']:
    rule seq_preprocessor_long_read:
        input:
    ...
    rule check_md5_long_read:
        input:
            ...
            link_se = expand(os.path.join('../00.raw_data',
                                          ...
                                          long_sample=long_read_samples.keys()),
        output:
            md5_check = "../01.qc/md5_check_long_read.tsv",
        message:
            "Running md5 check on raw data files on {input.md5_check_json}",
        benchmark:
            "../benchmarks/md5_check_long_read_benchmark.txt",
        log:
            "../logs/01.qc/md5_check_long_read.log",
        params:
            md5_check = os.path.join('../00.raw_data',config['long_read_convert_md5']),
            log_file = "../logs/01.qc/md5_check_long_read.log",
        threads: 
            config['threads']['long_md5_check'],
        shell:
            """
            ./scripts/json_md5_verifier/target/release/json_md5_verifier -t  {threads} \
                    -i {input.md5_check_json} \
